...
func testServerAPI(app *App) func(t *testing.T) {
	return func(t *testing.T) {
		...
		token, errToken := newServerToken(testServer.ServerID, app.settings.HTTP.CookieKey)
		...
		t.Run("sm_admins", func(t *testing.T) {
			...
			for _, perm := range perms {
				if steamid.SIDToSID64(perm.SteamID) == app.settings.General.Owner {
					ownerFound = true
					break
				}
			}
		...
		t.Run("sm_ban", func(t *testing.T) {
			...
			req := newTestReq(http.MethodPost, "/api/sm/bans/steam/create", apiBanRequest{
				SourceID:       store.StringSID(app.settings.General.Owner.String()),
