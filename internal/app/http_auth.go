...
func authServerMiddleWare(app *App) gin.HandlerFunc {
	...
	return func(ctx *gin.Context) {
		...
		parsedToken, errParseClaims := jwt.ParseWithClaims(reqAuthHeader, claims, makeGetTokenKey(app.settings.HTTP.CookieKey))
...
func onOpenIDCallback(app *App) gin.HandlerFunc {
	...
	return func(ctx *gin.Context) {
		...
		fullURL := app.settings.General.ExternalURL + ctx.Request.URL.String()

		if app.settings.Debug.SkipOpenIDValidation {
		...
		tokens, errToken := makeTokens(ctx, app.db, app.settings.HTTP.CookieKey, sid, true)
		if errToken != nil {
			ctx.Redirect(302, referralURL)
		...
		ctx.SetCookie(fingerprintCookieName, tokens.fingerprint, int(refreshTokenDuration.Seconds()), "/api", parsedExternal.Hostname(), app.settings.General.Mode == ReleaseMode, true)
	...
	return func(ctx *gin.Context) {
		...
		if errToken != nil || hdrToken == "" {
			...
			if level >= consts.PGuest {
				sid, errFromToken := sid64FromJWTToken(token, app.settings.HTTP.CookieKey)
				if errFromToken != nil {
